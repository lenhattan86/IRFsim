...
% beta = [33 40 40 96 512]/32;  % 32 cores per node vs 1 GPU 
beta = [33 40 40 96 112]/32;  % 32 cores per node vs 1 GPU 
...
if USE_TRACE
    ...
    arrival_dist = truncate(pd_normal,0,1400);
...
%         gpu_time = min(400,cpu_time/beta_job);
    beta_job = beta_mat(ceil(jobIdx/NJobs),mod(jobIdx-1,NJobs)+1);
     ...
     if (beta_job < 1)
         gpu_time = gpu_time / 1.5;
         cpu_time = cpu_time / 1.5;
     end
