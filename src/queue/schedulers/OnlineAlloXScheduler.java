...
public class OnlineAlloXScheduler implements Scheduler {
	...
	public OnlineAlloXScheduler() {
		clusterTotCapacity = Simulator.cluster.getClusterMaxResAlloc();
		this.schedulePolicy = "OnlineAlloX";
	}
	...
	public static void allox(Resource resCapacity, List<JobQueue> runningQueues) {
		...
		while (flag) {
			if (currLoad.resources[0] <= currLoad.resource(1)) {
				double Y = (currLoad.resource(1) - currLoad.resource(0))/(betas[gpumin]/resCapacity.resource(0) + 1.0/resCapacity.resource(1));
...
//			shares[0] = useralloc[i].resource(0) * resCapacity.resource(0) / sumAlloc.resource(0);
//			shares[1] = useralloc[i].resource(1) * resCapacity.resource(1) / sumAlloc.resource(1);
