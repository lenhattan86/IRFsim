...
public class PricingScheduler implements Scheduler {
	...
	public static void pricing(Resource resCapacity, List<JobQueue> runningQueues) {
		...
		price[0] = 1; price[1] = lastQueue.getReportBeta();	price[2] = 1 + lastQueue.getReportBeta();
		...
		if (currLoad.resources[1] > currLoad.resources[0]) {
			...
			currLoad = Resources.sum(finalAlloc); // x+beta y = z/ri;  cur_load(1)+x = cur_load(2)+ y;
		...
		while(Math.abs(currLoad.resources[0]- currLoad.resources[1])>error && flag){
			...
			if (currLoad.resources[0]>currLoad.resource(1)){
				...
				if(currLoadG.resources[0] > currLoadG.resource(1)){
					...
					finalAlloc[gpumin].resources[1] = (currLoadG.resources[0]-currLoadG.resources[1] + (finalAlloc[gpumin].resource(2)/ratios[gpumin]))/(1+betas[gpumin]);  
					finalAlloc[gpumin].resources[0] = currLoadG.resource(1)+finalAlloc[gpumin].resource(1) - currLoadG.resource(0);
					break;
				}
			}
			else
	...
	private static Resource[] useralloc(double betas[], double ratios[], double currentPrices[]){
		...
		Resource userAlloc[] = Resources.NONEs(betas.length);
		for(int j=0; j< betas.length; j++){
			userAlloc[j].resources[2] = Math.min(1/currentPrices[2], Math.max(ratios[j], betas[j]*ratios[j]/currentPrices[1]));
